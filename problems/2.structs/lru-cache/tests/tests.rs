...
fn it_works2() {
    let mut cache = LRUCache::new(2).unwrap();
    assert_eq!(cache.get(&2), None);
    assert_eq!(cache.put(2, 6), None);
    assert_eq!(cache.get(&1), None);
    assert_eq!(cache.put(1, 5), None);
    assert_eq!(cache.put(1, 2), Some(5));
...
fn it_works3() {
    let mut cache = LRUCache::new(2).unwrap();
    assert_eq!(cache.put(2, 1), None);
    assert_eq!(cache.put(2, 2), Some(1));
    assert_eq!(cache.get(&2), Some(&2));
    assert_eq!(cache.put(1, 1), None);
    assert_eq!(cache.put(4, 1), None);
    assert_eq!(cache.get(&2), None);
}
...
fn small_capacity() {
    ...
    for _ in 0..500000 {
        if rng.random_range(0..3) == 0 {
            ...
            assert_eq!(cache.put(key, value), naive.insert(key, value));
        }
    }
}
...
#[timeout(4000)]
fn stress() {
    let mut cache = LRUCache::new(100000).unwrap();
    let mut rng = rand::rng();
    for _ in 0..500000 {
        if rng.random_range(0..3) == 0 {
